...
function setup() {
  ...
  console.log("Width", width)
    invisibleGround = createSprite(width/2,height -100,width,90);  
 
 // invisibleGround.shapeColor = "#f4cbaa";
  
  ground = createSprite(width/2,height -80,width,90);
  ground.addImage("ground",groundImage);
  //image(groundImage,width/2,height-200,width,20);
...
function draw() {
  ...
  text("HighScore:" + highScore,70,50)
  
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
    
    if((touches.length> 0 || keyDown("SPACE")) && trex.y >=280)    
    {
      jumpSound.play( );
      trex.velocityY = -10;
      touches = [];
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(width +10,height -170,20,30);
